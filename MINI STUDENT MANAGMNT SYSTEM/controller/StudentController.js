...
$('#student_tbl_body').on('click', 'tr', function () {
    ...
    let student_detail = get_student(selectedIndex);

    $("#f_name").val(student_detail.f_name);
    $("#l_name").val(student_detail.l_name);
    $("#address").val(student_detail.address);

    console.log("Selected index:", selectedIndex, student_detail);
});

//============DELETE STUDENT=================//
$('#student_delete_btn').on("click", () => {
    if (selectedIndex === -1) {
        Swal.fire("Error!", "Please select a student to delete.", "error");
        return;
    }
...
$('#student_update_btn').on("click", function () {
    ...
    update_student(selectedIndex, updated_student);
